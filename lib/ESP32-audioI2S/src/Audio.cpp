...
bool Audio::elevenlabs_speech(const String& api_key, const String& voice_id, const String& input) {
    char host[] = "api.elevenlabs.io";
    String path = "/v1/text-to-speech/" + voice_id + "/stream?optimize_streaming_latency=2";
    ...
    // Force the multilingual model so it reads Hindi perfectly
    String post_body = "{\"text\": \"" + input_clean + "\", \"model_id\": \"eleven_multilingual_v2\"}";
    ...
    if (res) {
        _client->print(http_request);
        m_expectedCodec = CODEC_MP3; // ElevenLabs streams MP3 by default
        setDatamode(HTTP_RESPONSE_HEADER);
        m_streamType = ST_WEBSTREAM;
    } else {
        AUDIO_INFO("Request %s failed!", host);
